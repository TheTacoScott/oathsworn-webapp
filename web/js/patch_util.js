...
// =============================================================================
//  RUNTIME: install conditional time trigger support on GameEngine
// =============================================================================

(function installConditionalTimeTriggerSupport() {

    // Guard against double-installation (e.g. if this file is somehow loaded twice).
    if (GameEngine.prototype._hasConditionalTimeTriggerPatch) return;
    GameEngine.prototype._hasConditionalTimeTriggerPatch = true;

    // -------------------------------------------------------------------------
    // Condition evaluator
    // Returns true when the given nextPositionToken satisfies the condition.
    // -------------------------------------------------------------------------
    function conditionMatches(nextPositionToken, condition) {
        if (condition.whenTokenInRange) {
            const min = condition.whenTokenInRange[0];
            const max = condition.whenTokenInRange[1];
            if (nextPositionToken >= min && nextPositionToken <= max) return true;
        }

        if (condition.whenTokenNotInRange) {
            const min = condition.whenTokenNotInRange[0];
            const max = condition.whenTokenNotInRange[1];
            if (nextPositionToken < min || nextPositionToken > max) return true;
        }

        if (condition.orTokenIs) {
            if (condition.orTokenIs.includes(nextPositionToken)) return true;
        }
    ...
    // -------------------------------------------------------------------------
    // Wrap _timePop to also check conditionalTimeTriggers after the standard
    // unconditional triggers have been checked.
    // -------------------------------------------------------------------------
    const originalTimePop = GameEngine.prototype._timePop;

    GameEngine.prototype._timePop = function(timeAdded) {
        // First: run the original logic (unconditional triggers).
        const result = originalTimePop.call(this, timeAdded);
        if (result !== -1) return result;

        // Second: check conditional triggers if this chapter has any.
        const conditionalTriggers = this.chapterData.conditionalTimeTriggers;
        if (!conditionalTriggers) return -1;
        ...
        for (let unit = 1; unit <= timeAdded; unit++) {
            time++;
            const condition = conditionalTriggers[time];
            if (!condition) continue;

            const npt = this._getInt('nextPositionToken');
            if (conditionMatches(npt, condition)) {
